...
// white override just like the channel chip beside it. `path` is always a
// locale-neutral, already-built route (callers pass things like
// `/watch/${id}` or `/c/${id}`) - shared links intentionally stay
// unprefixed/current-locale-free, matching how /c/[id] itself lives outside
// locale routing (i18n plan goal #3).
...
export function WhatsAppShareButton({ title, path }: { title: string; path: string }) {
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className={CHIP_CLASS} aria-label={t("whatsappAria")}>
